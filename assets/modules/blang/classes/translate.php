...
class  translate
{
    ...
    private function translatePageBuilder($docId)
    {
        ...
        foreach ($languages as $language) {
            if ($language == $originalContainer) {
                continue;
            }

            $this->modx->db->delete($table,'`document_id` = '.$docId.' and `container` = "'.$language.'"');
            ...
            foreach ($blocks as $block) {
                ...
                $response = $this->translatePageBuilderFields($fields, $values, $originalContainer, $language);




                $block['values'] = json_encode($response,JSON_UNESCAPED_UNICODE);
                $block['container'] = $language;


                unset($block['id']);

                $block = $this->modx->db->escape($block);
                $resp = $this->modx->db->insert($block,$table);
